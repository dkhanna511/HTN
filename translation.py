...
Original file is located at
    https://colab.research.google.com/drive/1-nvS81CX1XBCLQmqhwBfZjWHsGHnQkzr
"""
...
def translate(model: torch.nn.Module, src_sentence: list):
    ...
    for i in range(len(src_sentence)):
        src = text_transform[SRC_LANGUAGE](src_sentence[i]).view(-1, 1)
        num_tokens = src.shape[0]
        src_mask = (torch.zeros(num_tokens, num_tokens)).type(torch.bool)
        tgt_tokens = greedy_decode(
            model,  src, src_mask, max_len=num_tokens + 5, start_symbol=BOS_IDX).flatten()
        result.append(" ".join(vocab_transform[TGT_LANGUAGE].lookup_tokens(list(tgt_tokens.cpu().numpy()))).replace("<bos>", "").replace("<eos>", ""))
    return result
...
def save_checkpoint(checkpoint, name= "translation_v2.pth.tar"):
